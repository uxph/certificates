...
const csvDirectory = path.join(__dirname, "data"); // Replace with your directory name
...
async function importAllCsvFiles() {
    try {
        ...
        for (const file of files) {
            ...
            // delete all first
            await db.collection("helixpay_event_attendees").doc().delete();

            console.log("Importing CSV to DB");
            const batch = db.batch();
            jsonArray.forEach((item) => {
                const docRef = db.collection("helixpay_event_attendees").doc(); // Change to doc(item.id) if needed
